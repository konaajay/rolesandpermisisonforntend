...
import { FiX, FiSettings, FiActivity, FiLayers, FiDollarSign, FiAlertTriangle, FiUserPlus, FiPlus, FiSend, FiLoader, FiCheckCircle } from 'react-icons/fi';
...
const AffiliateDetails = ({ affiliate, onClose }) => {
    ...
    const [leadFormData, setLeadFormData] = useState({ name: '', email: '', mobile: '' });
    ...
    const handleLeadSubmit = async (e) => {
        e.preventDefault();
        if (!selectedLinkForLead) return;
        
        setSubmittingLead(true);
        try {
            await affiliateService.submitLead({
                ...leadFormData,
                referralCode: selectedLinkForLead.referralCode,
                batchId: selectedLinkForLead.batchId,
                courseId: selectedLinkForLead.courseId
            });
            alert('Lead submitted successfully!');
            setSelectedLinkForLead(null);
            setLeadFormData({ name: '', email: '', mobile: '' });
            fetchData(); // Refresh metrics
        } catch (err) {
            alert('Failed to submit lead: ' + (err.response?.data?.message || err.message));
        } finally {
                                ...
                                {links.length === 0 ? (
                                    ...
                                    <div className="table-responsive border rounded">
                                        <table className="table table-hover align-middle mb-0 text-nowrap">
                                            ...
                                            <tbody>
                                                {links.map((link, idx) => {
                                                    const fullLink = `${window.location.host}/apply?ref=${link.referralCode}`;
                                                    const isFormOpen = selectedLinkForLead?.id === link.id;
                                                    return (
                                                        <React.Fragment key={link.id || idx}>
                                                            <tr className={isFormOpen ? 'table-primary border-primary border-opacity-25' : ''}>
                                                            ...
                                                            <td className="text-end">
                                                                <button 
                                                                    className={`btn btn-sm ${isFormOpen ? 'btn-danger' : 'btn-primary'} py-1 px-2 rounded-3 d-flex align-items-center gap-1 ms-auto`}
                                                                    onClick={() => setSelectedLinkForLead(isFormOpen ? null : link)}
                                                                >
                                                                    {isFormOpen ? <FiX /> : <FiUserPlus />} {isFormOpen ? 'Cancel' : 'Submit Lead'}
                                                                </button>
                                                            </td>
                                                        </tr>
                                                        {isFormOpen && (
                                                            <tr className="bg-primary bg-opacity-10 border-top-0">
                                                                <td colSpan="7" className="p-0 border-0">
                                                                    <div className="p-3 bg-white border border-primary border-opacity-25 rounded-4 m-2 shadow-sm animate-fade-in">
                                                                        <form onSubmit={handleLeadSubmit} className="row g-2 align-items-end">
                                                                            <div className="col-md-3">
                                                                                <label className="x-small text-muted fw-bold mb-1">STUDENT NAME</label>
                                                                                <input 
                                                                                    type="text" 
                                                                            ...
                                                                            <div className="col-md-3">
                                                                                <label className="x-small text-muted fw-bold mb-1">EMAIL</label>
                                                                                <input 
                                                                                    type="email" 
                                                                                    className="form-control form-control-sm bg-light" 
                                                                            ...
                                                                            <div className="col-md-3">
                                                                                <label className="x-small text-muted fw-bold mb-1">MOBILE</label>
                                                                                <input 
                                                                                    type="tel" 
                                                                                    className="form-control form-control-sm bg-light" 
                                                                            ...
                                                                            <div className="col-md-3">
                                                                                <button 
                                                                                    type="submit" 
                                                                                    className="btn btn-sm btn-primary w-100 py-1 fw-bold rounded-3 d-flex align-items-center justify-content-center gap-2"
                                                                                    disabled={submittingLead}
                                                                                >
                                                                                    {submittingLead ? <FiLoader className="spinner-border spinner-border-sm border-0" /> : <FiSend />}
                                                                                    {submittingLead ? 'SUBMITTING...' : 'GO !'}
                                                                                </button>
                                                                            </div>
                                                                        </form>
                                                                        <div className="mt-2 x-small text-muted">
                                                                            <FiActivity className="me-1" /> Attribute manual lead to code: <span className="fw-bold">{link.referralCode}</span>
                                                                        </div>
                                                                    </div>
                                                                </td>
                                                            </tr>
