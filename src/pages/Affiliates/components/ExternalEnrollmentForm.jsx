...
const ExternalEnrollmentForm = () => {
  ...
  */
  ...
  const fetchCourses = () => {
    setCourses([]); // Disabled to avoid 400 errors
  };
  ...
  // 3. Submit Lead to Central Backend
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("[ExternalEnrollmentForm] Sending Lead Payload (v18):", JSON.stringify(payload));
      const response = await leadService.createLead(payload);
      console.log("[ExternalEnrollmentForm] Success:", response);

      if (response && response.id) {
        window.alert("SUCCESS! External lead captured.");
        setStatus({ 
          type: 'success', 
          message: 'Success! Your interest has been registered.' 
        });
        setFormData({ firstName: '', lastName: '', phone: '', email: '', courseId: '', batchId: '' });
      }
    } catch (error) {
      console.error("[ExternalEnrollmentForm] Submission failed:", error);
