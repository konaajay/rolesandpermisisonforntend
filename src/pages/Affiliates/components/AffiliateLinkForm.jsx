...
const courseService = { getCourses: async () => [] };
const batchService = { getBatchesByCourseId: async () => [] };
const userService = { getAllUsers: async () => [] };
...
const AffiliateLinkForm = ({ onSave, onCancel, initialAffiliate = null }) => {
    ...
    if (generatedLink) {
        return (
            <div className="p-4">
                ...
                <a href={generatedLink.link} target="_blank" rel="noreferrer" className="btn btn-premium-primary w-100 mb-2">
    ...
    return (
        <form onSubmit={handleSubmit} className="d-flex flex-column" style={{ maxHeight: '85vh' }}>
            ...
            {/* ── Sticky Footer ── */}
            <div className="px-4 py-3 border-top bg-white flex-shrink-0 d-flex align-items-center justify-content-between gap-3 flex-wrap">
                <div className="small text-muted mb-1 mb-sm-0">
                    Partner: <span className="fw-semibold text-dark">{selectedAffiliate?.name || '—'}</span>
                </div>
                <div className="d-flex gap-2 w-100 w-sm-auto">
                    <button type="button" className="btn btn-outline-secondary btn-sm px-4 fw-semibold flex-grow-1 flex-sm-grow-0" style={{ borderRadius: '12px' }} onClick={onCancel}>
                        Cancel
                    </button>
                    <button type="submit"
                        className="btn btn-premium-primary btn-sm px-4 fw-semibold flex-grow-1 flex-sm-grow-0"
                        disabled={submitting}>
                        {submitting ? 'Generating...' : (initialAffiliate ? 'Update Link' : 'Generate Link')}
                    </button>
                </div>
            </div>
