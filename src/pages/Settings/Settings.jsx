...
import {
  fetchAllSettings,
  updatePlatformSettings as savePlatform,
  updateSecuritySettings as saveSecurity,
  updateCommunicationSettings as saveCommunication,
  updateGeneralSettings as saveGeneral,
  replaceCustomFields as saveCustomFields
...
const Settings = () => {
  ...
  // --- State slices matching backend response shape ---
  const [generalSettings, setGeneralSettings] = useState({
    logo: '', siteName: '', language: 'en', timezone: 'UTC'
  });

  const [domainSettings, setDomainSettings] = useState({
    customDomain: '', cloudflare: false
  });

  const [paymentSettings, setPaymentSettings] = useState({
    currency: 'USD ($)', taxType: 'None', foreignPricing: false,
    taxId: '', bankAccount: '', ifsc: ''
  });

  const [taxSettings, setTaxSettings] = useState({
    enableInvoices: true, legalName: '', taxIdLabel: 'GSTIN',
    address: '', prefix: 'INV-', serial: 1001, footerNote: ''
  });

  const [securitySettings, setSecuritySettings] = useState({
    maxDevices: 2, watermarking: true, showEmail: true,
    showPhone: true, showIp: false, admin2fa: false
  });

  const [authSettings, setAuthSettings] = useState({
    googleLogin: true, passwordPolicy: 'Standard', doubleOptIn: false
  });

  const [uxSettings, setUxSettings] = useState({
    verification: true, communication: true, welcomeMessage: ''
  });

  const [commSettings, setCommSettings] = useState({
    senderName: 'LMS Academy Team', replyTo: ''
  });
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      if (data.uxSettings) setUxSettings(data.uxSettings);
      if (data.commSettings) setCommSettings(data.commSettings);
  ...
  const handleSavePlatform = async () => {
    ...
    try {
      const payload = {
        ...
        ...paymentSettings,
        ...taxSettings,
        serial: Number(taxSettings.serial)
      };
  ...
  const handleSaveSecurity = async () => {
    ...
    try {
      const payload = {
        ...securitySettings,
        ...authSettings,
        maxDevices: Number(securitySettings.maxDevices)
      };
      await saveSecurity(payload);
      await reloadSettings();
  ...
  const handleSaveCommunication = async () => {
    setSaving(true);
    try {
      const payload = { ...commSettings, ...uxSettings };
      await saveCommunication(payload);
      await reloadSettings();
  ...
  const handleSaveGeneral = async () => {
    setSaving(true);
    try {
      await saveGeneral(generalSettings);
      await reloadSettings();
  ...
  const handleSaveCustomFields = async (fieldsToSave) => {
    setSaving(true);
    try {
      await saveCustomFields(fieldsToSave);
  ...
  const handleAddCustomField = () => {
    ...
    if (label) {
      const newFields = [
        ...customFields,
        { id: label.toLowerCase().replace(/\s+/g, '_'), label, type: 'Text', icon: 'file' }
      ];
      ...
      case 'general':
        return (
          <GeneralSettings
            ...
            update={updateState(setGeneralSettings)}
      ...
      case 'platform':
        return (
          <div className="d-flex flex-column gap-5">
            <DomainSettings
              data={domainSettings}
              update={updateState(setDomainSettings)}
              onSave={handleSavePlatform}
              saving={saving}
            />
            <PaymentSettings
              data={paymentSettings}
              update={updateState(setPaymentSettings)}
              onSave={handleSavePlatform}
              saving={saving}
            />
            <TaxSettings
              data={taxSettings}
              update={updateState(setTaxSettings)}
              onSave={handleSavePlatform}
              saving={saving}
            />
          </div>
        ...
        return (
          <div className="d-flex flex-column gap-5">
            <SecuritySettings
              ...
              update={updateState(setSecuritySettings)}
              onSave={handleSaveSecurity}
              saving={saving}
            />
            <SignupSettings
              data={authSettings}
              update={updateState(setAuthSettings)}
              onSave={handleSaveSecurity}
              saving={saving}
            />
          </div>
        ...
        return (
          <div className="d-flex flex-column gap-5">
            <CommunicationSettings
              data={commSettings}
              update={updateState(setCommSettings)}
              onSave={handleSaveCommunication}
              saving={saving}
            />
            <UXSettings
              data={uxSettings}
              update={updateState(setUxSettings)}
              onSave={handleSaveCommunication}
              saving={saving}
            />
  ...
  if (loading) {
    return (
      <div className="settings-container d-flex align-items-center justify-content-center" style={{ minHeight: '80vh' }}>
        <div className="text-center">
          <Loader size={40} className="text-indigo-500 spin-animation" />
...
const GeneralSettings = ({ data, update, onSave, saving }) => (
  <div className="settings-card">
    <SectionHeader
      icon={<Layout size={24} />}
      title="General Settings"
      subtitle="Customize your academy's brand and basic information."
    />
    <div className="row g-4">
      ...
      <div className="col-md-12">
        ...
        <div className="d-flex gap-3 align-items-center">
          ...
          />
          {data.logo && (
            <div className="bg-light p-2 rounded border" style={{ width: 60, height: 60 }}>
              <img src={data.logo} alt="Preview" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
            </div>
          )}
        </div>
      </div>
      <div className="col-md-6">
        ...
        <select className="select-field" value={data.language} onChange={(e) => update('language', e.target.value)}>
      ...
      <div className="col-md-6">
        ...
        <select className="select-field" value={data.timezone} onChange={(e) => update('timezone', e.target.value)}>
...
const DomainSettings = ({ data, update, onSave, saving }) => (
  <div className="settings-card">
    <SectionHeader
      icon={<Globe size={24} />}
    ...
    <div className="row g-4">
      ...
      <div className="col-12 d-flex justify-content-end mt-4">
        <SaveButton onClick={onSave} saving={saving} label="Verify & Save" />
      </div>
    </div>
  </div>
);

const PaymentSettings = ({ data, update, onSave, saving }) => (
  <>
    <div className="settings-card">
    ...
    <div className="settings-card">
      ...
      <div className="row g-4">
        ...
        <div className="col-12 text-end">
          <SaveButton onClick={onSave} saving={saving} label="Submit for Verification" />
        </div>
      </div>
    </div>
  </>
...
const TaxSettings = ({ data, update, onSave, saving }) => (
  <div className="settings-card">
...
const SecuritySettings = ({ data, update, onSave, saving }) => (
  <div className="settings-card">
...
const SignupSettings = ({ data, update, onSave, saving }) => (
  <div className="settings-card">
    ...
    <div className="col-12 d-flex justify-content-end mt-5">
      <SaveButton onClick={onSave} saving={saving} label="Save Configuration" />
    </div>
  </div>
);

const CommunicationSettings = ({ data, update, onSave, saving }) => (
  <>
    <div className="settings-card">
...
const UXSettings = ({ data, update, onSave, saving }) => (
  <div className="settings-card">
      ...
      <div key={field.id} className={`p-4 rounded-xl mb-3 d-flex justify-content-between align-items-center ${field.type === 'Mandatory' ? 'bg-orange-50 border border-orange-100' : 'bg-white border'}`}>
        <div className="d-flex gap-3 align-items-center">
          <div className={`${field.type === 'Mandatory' ? 'text-orange-500' : 'text-slate-400'}`}>
            {field.icon === 'smartphone' ? <Smartphone size={20} /> : <FileText size={20} />}
          </div>
          <div>
